...
// Outputs all AppIcon PNG sizes to Cadence/Assets.xcassets/AppIcon.appiconset/
import AppKit
...
let dest = "Cadence/Assets.xcassets/AppIcon.appiconset"
...
for (size, name) in sizes {
    let path = "\(dest)/\(name)"
    try! renderIcon(size: size).write(to: URL(fileURLWithPath: path))
    print("Wrote \(path)")
}
